...
# Função para mostrar pratos por categoria
def mostrar_cardapio_por_categoria(categoria):
    dados = carregar_dados()
...
# Função para atualizar apenas os ingredientes de um prato
def atualizar_ingredientes():
    dados = carregar_dados()
    id_prato = int(input("Digite o ID do prato que deseja atualizar os ingredientes: "))
    prato = next((p for p in dados if p['id'] == id_prato), None)
    ...
    if prato:
        ...
        salvar_dados(dados)
        print("Ingredientes do prato atualizados com sucesso!")
    else:
        print("Prato não encontrado.")
...
def exibir_menu():
    ...
    print("2. MOSTRAR PRATO POR CATEGORIA")
    print("3. ATUALIZAR PRATO")
    print("4. ATUALIZAR INGREDIENTE DE UM PRATO")
    print("5. REMOVER PRATO")
    print("6. SAIR")
    print('-'*20)
...
def main():
    while True:
        ...
        if opcao == '1':
            ...
            categoria = selecionar_categoria()
            mostrar_cardapio_por_categoria(categoria)
        elif opcao == '3':
            atualizar_prato()
        elif opcao == '4':
